...
my_vars <- tribble(
  ~variable, ~sn,
  "B25026_001", "pop_ohu", # Total population in occupied housing units, for tract-level
  "B19013_001", "hhincome", 
...
For a model you might use e.g. an encounter-level table of data with tract-level or other features attributed to each case. Or alternatively e.g. a tract-level analysis with some summary of the number or type of encounters merged in. That's why we do the spatial join above. But for this map we'll need both tables---the  `la_tracts` table and the `la_pip_clean` table. This is because one has the tract map shapes and the other has the point locations of the FEs. The base tract map looks like this:
