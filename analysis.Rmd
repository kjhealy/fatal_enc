...
But we still need to check! Let's drop all the rows with missing values on `tract`, which should give us all and only FEs in LA County tracts.  
...
This might be easy to miss in a join because the Lat/Lon doesn't correspond to an LA County tract. The row would still be in the datat but if we droped on `tract` after the join we'd lose that case. But that it is in fact a Fatal Encounter that happened in an LA County tract. I'm going to ignore it for now. But this is the sort of thing you have to be careful of. And also, I don't like it that I found a bad geo record in the FE data on the very first random pass through. 
...
For a model you might use e.g. an encounter-level table of data with tract-level or other features attributed to each case. Or alternatively e.g. a tract-level analysis with some summary of the number or type of encounters merged in. But for this map we'll need both tables---the  tract table and the `la_pip_clean` table. This is because one has the tract map shapes and the other has the point locations of the FEs. The base tract map looks like this:
...
We're going to put the points on top in a separate layer. The two weird tracts in the south are Santa Catalina Island and San Clemente Island. Let's get rid of those. 
